...
const flaresolverr = require('./flaresolverr').instance;
...
async function getCloudflareCookie(forceRefresh = false) {
    ...
    // Try FlareSolverr first if enabled (with multiple attempts)
    if (flaresolverr.isEnabled()) {
        console.log('[Cloudflare] Tentando bypass con FlareSolverr...');
        
        // Tentativo 1: Homepage con timeout breve
        try {
            console.log('[Cloudflare] FlareSolverr attempt 1: Homepage with short timeout...');
            process.env.FLARESOLVERR_TIMEOUT = '60000';
            const homepageCookie = await flaresolverr.getCloudflareCookie('https://kisskh.co/');
            if (homepageCookie) {
                const cookieValue = homepageCookie.replace('cf_clearance=', '');
                cache.set('cf', homepageCookie);
                await saveCookieToFile(cookieValue);
                console.log('[Cloudflare] FlareSolverr homepage bypass successful!');
                return homepageCookie;
            }
        } catch (error) {
            console.warn('[Cloudflare] FlareSolverr homepage attempt failed:', error.message);
        }
        
        // Tentativo 2: API con timeout lungo
        try {
            console.log('[Cloudflare] FlareSolverr attempt 2: API endpoint with long timeout...');
            process.env.FLARESOLVERR_TIMEOUT = '180000';
            const flareCookie = await flaresolverr.getCloudflareCookie('https://kisskh.co/api/DramaList/List?page=1&type=0&sub=0&country=0&status=0&order=1&genre=0');
            if (flareCookie) {
                const cookieValue = flareCookie.replace('cf_clearance=', '');
                cache.set('cf', flareCookie);
                await saveCookieToFile(cookieValue);
                console.log('[Cloudflare] FlareSolverr API bypass successful!');
                return flareCookie;
            }
        } catch (error) {
            console.warn('[Cloudflare] FlareSolverr API attempt failed:', error.message);
        }
        
        console.log('[Cloudflare] All FlareSolverr attempts failed, falling back to Puppeteer...');
    } else {
        console.log('[Cloudflare] FlareSolverr disabled, using Puppeteer...');
    }
    
    // Fallback to original Puppeteer method
    return await getCloudflareCookieWithPuppeteer();
}

async function getCloudflareCookieWithPuppeteer() {
...
module.exports = {
    getCloudflareCookie,
    getCloudflareCookieWithPuppeteer
};
